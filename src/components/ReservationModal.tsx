      ...
      <div className="relative w-full max-w-2xl bg-[#1c040d] border border-[#6b152d] text-[#f5f1ea] rounded-3xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="px-6 py-5 border-b border-[#521324] flex items-center justify-between bg-[#24060f]">
          <div>
            ...
            <h3 className="font-serif text-sm sm:text-base text-white/80 font-medium">Table Reservation</h3>
          </div>
          <button
            onClick={onClose}
          ...
          {isSuccess ? (
            <div className="text-center py-8 px-4 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-[#f3d2d8] max-w-md mx-auto">
              ...
              <div className="max-w-md mx-auto bg-[#24060f] border border-[#6b152d] rounded-2xl p-5 text-left text-xs space-y-2.5 text-[#f5f1ea]">
                <div className="flex justify-between border-b border-[#521324] pb-2">
                  <span className="text-[#f5d79e]/70">Date &amp; Time:</span>
                  <span className="font-mono text-white font-bold">{date} at {time}</span>
                </div>
                <div className="flex justify-between border-b border-[#521324] pb-2">
                  <span className="text-[#f5d79e]/70">Party Size:</span>
                  <span className="font-bold text-white">{guests} {guests === 1 ? 'Guest' : 'Guests'}</span>
                </div>
                <div className="flex justify-between border-b border-[#521324] pb-2">
                  <span className="text-[#f5d79e]/70">Seating Area:</span>
                  <span className="font-bold text-[#d4a359]">{seatingOptions.find(s => s.id === seatingArea)?.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#f5d79e]/70">Location:</span>
              ...
              <button
                onClick={handleReset}
                className="px-8 py-3 rounded-full bg-[#d4a359] text-[#180309] font-bold text-xs uppercase tracking-widest hover:bg-[#e2b775] transition-transform hover:scale-105"
              >
                Done
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Party Size Selector */}
              <div>
                <label className="block text-xs uppercase tracking-wider text-[#f5d79e] font-mono mb-2 flex items-center space-x-1.5">
                  ...
                  {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
                    <button
                      ...
                      className={`w-11 h-11 rounded-xl text-sm font-bold flex items-center justify-center transition-all ${
                        guests === num
                          ? 'bg-[#d4a359] text-[#180309] shadow-lg scale-105'
                          : 'bg-[#24060f] text-[#f3d2d8] hover:bg-[#3d0917]'
                      }`}
                    >
                      {num}
                    </button>
                  ...
                  {seatingOptions.map((seat) => (
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border cursor-pointer transition-all ${
                        seatingArea === seat.id
                          ? 'bg-[#2d0713] border-[#d4a359] shadow-md ring-1 ring-[#d4a359]/30'
                          : 'bg-[#24060f] border-[#521324] hover:border-[#831f3b]'
                      }`}
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-base">{seat.icon}</span>
                        <span className="text-[9px] px-2 py-0.5 rounded-full bg-[#3d0917] text-[#f5d79e] font-mono font-semibold">
                          {seat.tag}
                        </span>
                      </div>
                      <h4 className="font-serif text-sm text-white font-medium">{seat.name}</h4>
                      <p className="text-[11px] text-[#f3d2d8] leading-tight mt-0.5">{seat.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                <h4 className="text-xs uppercase tracking-wider text-[#f5d79e] font-mono">Guest Details</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input
                    type="text"
                    ...
                    required
                    className="w-full bg-[#24060f] border border-[#6b152d] rounded-xl px-4 py-2.5 text-xs text-white placeholder-[#f5d79e]/50 focus:outline-none focus:border-[#d4a359]"
                  />
                  <input
                    type="tel"
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-[#24060f] border border-[#6b152d] rounded-xl px-4 py-2 text-xs text-white placeholder-[#f5d79e]/50 focus:outline-none focus:border-[#d4a359]"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-[#d4a359] to-[#b8863b] text-[#180309] font-bold text-xs uppercase tracking-[0.2em] shadow-lg hover:scale-[1.01] transition-transform cursor-pointer"
              >
